...
LeftVisMtcnn=np.array([0,1,2,3,4],dtype='int')
LeftVis=np.array([38,43,30,48,54],dtype='int')
...
def poseEstimation(img, pt2d):
    ...
    #print('LeftVis')
    #pprint.pprint(LeftVis)

    #print('RightVis')
    #pprint.pprint(RightVis)
    ...
    if abs(gammal) > abs(gammar):
        ...
        gamma = gammar
        theta = thetar
    print('final ====\n{}{}{}'.format(phi, gamma, theta))
        
    return([phi, gamma, theta])



def poseEstimation5pt(img, pt2d):

    phil, gammal, thetal, t3d, f = PoseEstimation(pt2d[:, LeftVisMtcnn], pt3d[:, LeftVis])
    #phir, gammar, thetar, t3d, f = PoseEstimation(pt2d[:, RightVis], pt3d[:, RightVis])
    phi = phil
    gamma = gammal
    theta = thetal
